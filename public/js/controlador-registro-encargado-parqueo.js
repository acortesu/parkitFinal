...
const input_correo = document.querySelector("#correo-enc-par");
...
let validar = () => {
  ...
  // VALIDAR MAIL
  if (mailformat.test(input_correo.value) == false) {
  ...
  if (
    ...
    calcular_edad(new Date(fecha_nacimiento.value)) < 18
  ) {
...
let obtener_datos = async () => {
  ...
  if (error) {
    ...
    const newEncParUser = {
      ...
      id_parqueo: window.location.search.substr(1),
    };
    const response = await registrar_encargado_parqueo_user(newEncParUser);
    if (response) {
      Swal.fire({
